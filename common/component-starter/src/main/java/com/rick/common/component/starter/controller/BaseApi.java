...
public class BaseApi<S extends BaseServiceImpl<? extends EntityDAO<T, ID>, T, ID>, T extends EntityId<ID>, ID> {
    ...
    @GetMapping("detail")
    public Grid<T> listCascade(HttpServletRequest request) {
        Grid<Map<String, Object>> grid = list(request);
        Set<ID> ids = grid.getRows().stream().map(m -> (ID)m.get(Constants.ID_COLUMN_NAME)).collect(Collectors.toSet());
        Grid<T> returnGrid = Grid.emptyInstance(0);
        if (CollectionUtils.isNotEmpty(ids)) {
            returnGrid = Grid.<T>builder().page(grid.getPage()).totalPages(grid.getTotalPages()).rows(entityDAO.selectByIds(ids)).records(grid.getRecords()).pageSize(grid.getPageSize()).build();
        }
